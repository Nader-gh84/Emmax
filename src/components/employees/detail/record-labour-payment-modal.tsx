...
import { planLabourFifoAllocations } from "@/lib/labour-accounting";
...
export function RecordLabourPaymentModal({
  open,
  invoices,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-4 sm:items-center">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="record-labour-payment-title"
      ...
      >
        <h2
          id="record-labour-payment-title"
          className="text-xl font-semibold text-white"
        >
          Record Payment
        </h2>
        <p className="mt-1 text-sm text-slate-400">
          Payments apply to the employee account. Leave invoices unchecked to
                      ...
                      <label className="flex cursor-pointer items-start gap-2 rounded-lg border border-white/10 px-2.5 py-2 text-sm hover:bg-white/[0.04]">
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="block text-xs text-slate-500">
                            Due {formatEmployeeDate(invoice.dueDate)} · Balance{" "}
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.02] px-3 py-3 text-xs text-slate-400">
                  ...
                  {previewPlan.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span>{formatEmployeeMoney(row.amountApplied)}</span>
                      </li>
                    );
                  })}
                </ul>
              ...
              {unallocated > 0.009 ? (
                <p className="mt-2 text-amber-200">
                  Unallocated credit: {formatEmployeeMoney(unallocated)}
                </p>
              ) : null}
            </div>
          ) : null}
